...
type private PostDataAggr =
    { title:       PostTitle option
      ...
      source:      PostText option }
    with
    static member Empty source =
        { title       = None 
          publishDate = None
          source      = Some (PostText source) }
    member x.Validate () =
           x.title      .IsSome
...
let private convertDate (source: string) =
    ...
    | (true, v) -> Ok (Some (PostPublishDate v))
...
let private parseProperty line properties =
    let m = System.Text.RegularExpressions.Regex.Match(line, "@(\w+) *: *([\s\w-,.;@\"'`\\/\(\)\[\]\{\}]+)")
...
let parseLine source properties =
...
let private applyProp prop (data: PostDataAggr) =
    ...
    | ("Date", v) -> applyPublishDate data v
...
let parsePost source =
    ...
    let (data, errors) = applyProps props (PostDataAggr.Empty rest, [])
